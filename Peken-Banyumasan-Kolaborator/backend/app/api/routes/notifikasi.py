...
def mark_read(notif_id: str, user: dict = Depends(get_current_user)) -> dict:
    return {"data": mark_notification_read(user, notif_id)}


@router.patch("/baca-semua")
def mark_read_all(user: dict = Depends(get_current_user)) -> dict:
    return {"data": mark_all_read(user)}

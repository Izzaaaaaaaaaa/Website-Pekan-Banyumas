...
### v2.4.2 — 2026-06-11 (Cross-domain consistency sweep — change-request, money aggregates, stateless manual visitors)

**DB live unchanged (drift CLOSED on paper: live DB already had `event_artisans.change_request`; schema.sql now documents it). OA all bumped 2.4.1 → 2.4.2. Backend fixes assigned to teams via `PLANS/*_plan.md` (wave 2026-06-11).**

| Item | Type | Detail |
|------|------|--------|
| S-1 | Doc (schema.sql) | `event_artisans.change_request TEXT` added to the schema file + COMMENT — column **already existed in the live DB** (verified via REST 2026-06-11; a real `pending_change` row exists). Canonical home of stand-change requests = the junction row. `artisan_requests.change_request` only serves the pre-approval queue. |
| S-2 | Doc (gate OA) | `GET /api/events/{id}/artisan-requests` description: response = UNION of `artisan_requests` (self-join queue) + `event_artisans` rows with `status_request='pending_change'` (id = junction id, posisi_event = current stand, change_request = requested stand). `PATCH .../{rid}/change`: `rid` = event_artisans.id; approve swaps `stand_id`←`change_request` + clears + status approved (never deletes); reject clears only. Gate BE implementation = gate team task. |
| S-3 | Doc (artisan OA) | `requestPositionChange` description corrected to match the actual implementation: writes to **event_artisans junction row**, not artisan_requests. Artisan BE code unchanged (it was already correct). |
| S-4 | Doc (colab OA) | `Event` schema: + `status_efektif` (backend-derived display status: selesai if past/selesai, berlangsung if today/berlangsung, else raw; never stored). `status` returned RAW — backend must NOT rename published→'upcoming' (BE currently violates this; kolab team task). `request_status` ('pending'/'approved'/null) was already in the spec — BE must implement it (currently sends `pending_request` bool). |
| S-5 | Doc (gate OA) | `POST /api/visitors/manual` is now **stateless**: masuk = INSERT open row; keluar = ALWAYS INSERT a closed row (never pair/close an existing row — old behavior could close an NFC visitor's session). NFC tap flow unchanged (stateful). `di_dalam` stat = NFC inside + max(0, manual masuk − manual keluar). Gate BE task. |
| S-6 | Doc (schema.sql) | Money invariants clarified: `artisans.total_penjualan = SUM(kas.nominal WHERE jenis='masuk')` and `komisi_terkumpul = ROUND(total_penjualan*komisi_persen/100, 2)`, computed by the ARTISAN BACKEND on every kas mutation (no DB trigger exists). `kas.saldo_after` = running balance ordered (tgl, created_at), persisted by artisan BE. **Live DB currently violates these** (all aggregates 0.00, saldo_after chain broken) — artisan BE fix + one-time backfill SQL in `PLANS/artisan_plan.md`. |
| S-7 | Version | All 4 OA `info.version` 2.4.1 → **2.4.2**. Backend `/health` version strings (4× `routers/health.py`) to be bumped by each team alongside their fixes. |

**Invariants preserved:** kategori_usaha/subsektor vocab, kas.metode (tunai|qris), events.status CHECK, posisi payload mapping, internal_notes gate-only — all untouched.

**Verification after this delta:** `bash db/verify_cross_consistency.sh` → check 9 expects v2.4.2 on all 4 OA (done). Re-run full 16/16 after the team backend fixes land.

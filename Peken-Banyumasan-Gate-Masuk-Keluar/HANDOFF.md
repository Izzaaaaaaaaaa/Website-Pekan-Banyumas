...
2. **total_penjualan invariant**: `artisans.total_penjualan == SUM(kas.nominal WHERE kas.artisan_id = artisans.id AND kas.jenis = 'masuk')`.
...
4. **Atomic updates**: Inserting/updating/deleting a kas row MUST trigger backend re-computation of `saldo_after`, `total_penjualan`, `komisi_terkumpul` in one transaction. Use service_role bypass for these writes.

...
import { dashboardApi, eventApi } from '../services/endpoints';
...
const Dashboard = () => {
  ...
  // Event Mendatang — real published events, soonest first (was hardcoded demo).
  useEffect(() => {
    eventApi.list()
      .then((rows) => {
        const upcoming = (rows || [])
  ...
  // Debounced refresh — a manual/tap action triggers an explicit refresh AND a
  // Supabase realtime echo (the row it just inserted); coalesce that burst into
  // a single fetch so the dashboard refreshes once, not twice.
  ...
  useEffect(() => {
    ...
    if (supabaseRealtime) {
      channel = supabaseRealtime
        .channel('dashboard-visitors-privacy')
        // Subscribe to the real table — it's `visitors`, not `kunjungan`. The
        // old name silently never fired, so cross-device sync (admin ↔ petugas)
        // and instant manual/tap feedback fell back to 5s polling only.
  ...
  }, [fetchActivities, fetchStats, refreshDashboard]);
    ...
    <div className="space-y-4 font-sans pb-1">
      ...
      <div className="bg-white rounded-2xl border border-[#e4e7d4] p-5">
        ...
        {upcomingEvents.length === 0 ? (
          <p className="text-sm text-[#8a9070] py-4 text-center">Belum ada event mendatang.</p>
        ) : (
        <div className="grid sm:grid-cols-3 gap-3">
          {upcomingEvents.map(ev => {
            const kapasitas = Number(ev.kapasitas) || 0;
            const peserta   = Number(ev.peserta_count) || 0;
            const pct       = kapasitas ? Math.round(peserta / kapasitas * 100) : 0;
            return (
            <div key={ev.id} className="bg-[#f7f8f2] border border-[#e4e7d4] rounded-xl p-4 hover:border-[#c8d09a] transition cursor-pointer" onClick={() => navigate('/events')}>
              ...
              <div className="mt-2">
                <div className="flex justify-between text-[10px] text-[#8a9070] mb-1">
                  <span>{peserta} peserta</span><span>{pct}%</span>
                </div>
                <div className="h-1 bg-gray-200 rounded-full overflow-hidden">
                  <div className="h-full bg-[#7a8a52] rounded-full" style={{width:`${Math.min(100, pct)}%`}}/>
                </div>
              </div>
            </div>
            );
          ...
          <div className="bg-white p-4 rounded-2xl border border-[#e4e7d4]">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <span className="text-white/80 text-[11px] mt-1">+1 pengunjung / klik</span>
              </button>

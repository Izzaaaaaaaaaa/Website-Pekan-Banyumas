        ...
        {exp && (
          <div className="mt-4 space-y-3">
            ...
            <div className="bg-gray-50 rounded-xl p-3 space-y-1.5 text-sm">
              <div className="flex items-center gap-2 text-gray-600"><Phone size={12}/>{t.no_hp}</div>
              <div className="flex items-center gap-2 text-gray-600"><Mail size={12}/>{t.email}</div>
              <div className="flex items-center gap-2 text-gray-600"><MapPin size={12}/>{t.kota}</div>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 mb-1.5 block flex items-center gap-1"><Percent size={11}/>Komisi Default (%)</label>
              <div className="flex items-center gap-3">
                <input type="range" min="0" max="30" value={komisi} onChange={e=>setKomisi(+e.target.value)} className="flex-1"/>
                <div className="w-14 border border-gray-200 rounded-lg px-2 py-1.5 text-center font-bold text-indigo-700 text-sm bg-indigo-50">{komisi}%</div>
              </div>
              <p className="text-[10px] text-gray-400 mt-1">Bisa diubah nanti di halaman detail artisan</p>
            </div>
          </div>
        )}
...
const ArtisanRow = memo(({ t, onEdit }) => (
  <tr className="border-b border-gray-50 hover:bg-gray-50/60 transition group">
    ...
    <td className="px-4 py-3.5">
      ...
      <p className="text-gray-400 text-xs flex items-center gap-1 mt-1"><MapPin size={10}/>{t.kota}</p>
    </td>
    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full sm:w-[420px] bg-white shadow-2xl z-50 flex flex-col" style={{animation:'slideIn .26s cubic-bezier(.32,.72,0,1) both'}}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center text-green-700 font-bold shrink-0">{artisan.nama_usaha.charAt(0)}</div>
            <div>
              <p className="font-bold text-gray-900 text-sm leading-tight">{artisan.nama_usaha}</p>
              <p className="text-gray-400 text-xs">{artisan.pemilik} · {artisan.kota}</p>
            </div>
          </div>
          ...
          {tab === 'usaha' && (
            <div className="p-5 space-y-5">
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 space-y-2">
                ...
                {[['Total Penjualan', fmtRupiah(artisan.total_penjualan),'text-gray-800'],
                  ['Komisi Terkumpul', fmtRupiah(artisan.komisi_terkumpul),'text-emerald-700'],
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2 text-gray-600"><Phone size={13}/>{artisan.no_hp}</div>
                <div className="flex items-center gap-2 text-gray-600"><Mail size={13}/>{artisan.email}</div>
              </div>
            </div>
          )}
          ...
          {tab === 'event' && (
            <div>
              <div className="px-5 py-3.5 border-b border-gray-50 flex items-center justify-between">
                <p className="text-sm text-gray-500">{artisanEvents.length} event</p>
          ...
          {tab === 'profil' && (
            <div className="p-5 space-y-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                <div className="flex justify-between text-sm"><span className="text-gray-500">Nama Usaha</span><span className="font-semibold text-gray-800">{artisan.nama_usaha}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-500">Kategori</span><span className="font-semibold text-gray-800">{artisan.kategori}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-500">Kota</span><span className="font-semibold text-gray-800">{artisan.kota}</span></div>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Deskripsi</p>
                <p className="text-gray-600 text-sm leading-relaxed">{artisan.deskripsi}</p>
              </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[['Total Artisan',aktif.length,'text-gray-800','bg-white'],
        ...
        <>
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <div className="px-4 py-2.5 flex items-center justify-between">
              <p className="text-xs text-gray-400">{filtered.length} dari {aktif.length} Artisan aktif</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <div className="px-5 py-3 border-t border-gray-50 text-xs text-gray-400">{filtered.length} Artisan aktif</div>
          </div>
        </>
      )}

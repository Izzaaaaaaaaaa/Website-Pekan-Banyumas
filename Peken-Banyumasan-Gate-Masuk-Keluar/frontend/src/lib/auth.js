 ...
 *     This means auto-refreshed tokens are automatically available to api.js.
 ...
 *               Venue-level config (zones, occupied state) is intentionally
 *               preserved — that data is device-scoped, not user-scoped.
 *
...
// Removes all per-user storage keys declared in storageKeys.js. Isolated
// into its own function so both onAuthStateChange and clearAuth() share it.
 ...
 * Venue-level config (zones, occupied state) is intentionally preserved —
 * that data belongs to the device/event, not the user.
 */
